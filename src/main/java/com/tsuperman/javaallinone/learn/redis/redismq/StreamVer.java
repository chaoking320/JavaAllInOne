...
public class StreamVer {
    public final static String STREAM_KEY = "rsm:";
    ...
    public StreamEntryID produce(String key, Map<String, String> message) {
        ...
        try {
            ...
            StreamEntryID id = jedis.xadd(STREAM_KEY + key, StreamEntryID.NEW_ENTRY, message);
            System.out.println("发布消息到" + STREAM_KEY + key + " 返回消息id=" + id.toString());
            return id;
        } catch (Exception e) {
    ...
    public void createCustomGroup(String key, String groupName, String lastDeliveredId) {
        ...
        try {
            ...
            jedis.xgroupCreate(STREAM_KEY + key, groupName, id, false);
    ...
    public List<Map.Entry<String, List<StreamEntry>>> consume(String key, String customerName, String groupName) {
        ...
        try {
            ...
            streams.put(STREAM_KEY + key, StreamEntryID.UNRECEIVED_ENTRY);
            List<Map.Entry<String, List<StreamEntry>>> result
                    = jedis.xreadGroup(groupName, customerName, xReadGroupParams, streams);
            System.out.println(groupName + "从" + STREAM_KEY + key + "接受消息, 返回消息：" + result);
            return result;
        } catch (Exception e) {
    ...
    public void ackMsg(String key, String groupName, StreamEntryID msgId) {
        ...
        try {
            ...
            System.out.println(STREAM_KEY + key + "，消费群组" + groupName + " 消息已确认");
        } catch (Exception e) {
    ...
    * */
    public boolean checkGroup(String key, String groupName) {
        ...
        try {
            jedis = jedisPool.getResource();
            List<StreamGroupInfo> xinfoGroupResult = jedis.xinfoGroup(STREAM_KEY + key);
    ...
    public void MqInfo(int type, String key, String groupName) {
        ...
        try {
            ...
            if (type == MQ_INFO_CONSUMER) {
                List<StreamConsumersInfo> xinfoConsumersResult = jedis.xinfoConsumers(STREAM_KEY + key, groupName);
                System.out.println(STREAM_KEY + key + " 消费者信息:" + xinfoConsumersResult);
                ...
                List<StreamGroupInfo> xinfoGroupResult = jedis.xinfoGroup(STREAM_KEY + key);
                System.out.println(STREAM_KEY + key + "消费者群组信息:" + xinfoGroupResult);
                for (StreamGroupInfo groupinfo : xinfoGroupResult) {
                ...
                StreamInfo xinfoStreamResult = jedis.xinfoStream(STREAM_KEY + key);
                System.out.println(STREAM_KEY + key + "队列信息:" + xinfoStreamResult);
